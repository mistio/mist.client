    ...
    if not cloud:
        ...
        for machine in machines:
            ...
            if pretty:
                ...
                print "%-25s %-60s %-10s %-20s %-30s %-20s" % (machine.name, machine.machine, machine.info['state'], ips,
...
def display_machine(machine):
    ...
    x.add_row([machine.name, machine.machine, machine.info['state'], ips, machine.cloud.title, tags])
    print x
